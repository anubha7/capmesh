...
class Controller {
    ...
    */
    ...
    async getEmail(uName)
    {
        let query={"userName":uName};
        let result=await dao.find("demo",query);
        let obj={"email":result[0].email}
        return obj;
    }
    async getMobile(uName)
    {
        let query={"userName":uName};
        let result=await dao.find("demo",query);
        let obj={"mobile":result[0].mobile}
        return obj;
    }
    ...
    async getDOB(uName)
    {
        let query={"userName":uName};
        let result=await dao.find("demo",query);
        let obj={" dateOfBirth":result[0].dateOfBirth}
        return obj;
    }
